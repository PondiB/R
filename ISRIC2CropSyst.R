...
#Author: John Mutua
#date created: 4th June 2016

require(raster) 
require(rgdal)

setwd("D:/ToBackup/Data/Projects/Water_Fund/ThikaChania/CropSyst_Script")
...
name="Farmer_sites"
p<-readOGR(shp,name)
p <- spTransform(p, crs(l_stack))

# Extract data from .tif files
df1 <- as.data.frame(extract(l_stack, p))
...
for (i in 1:nrow(df1)){
  ...
  # # Set the hydrologic group based on USDA soil textural classification system - TBC
  # hg <- df1$Sasumua_SoilTexture[i]
  # if(hg)
  #   
  #   {
  #   hg <= 4 <-"D"
  ...
  # }
  # 
  # Calculate the hydrologic condition
  ...
  # write.table(paste0("hydrologic_condition=",hg2), file, append = TRUE, row.names=FALSE, quote=FALSE, col.names=FALSE)
