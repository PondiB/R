...
For more details on source of MODIS data see <http://pekko.geog.umd.edu/usda/beta/data_new.php>. Otherwise,
download the sample dataset we will use in this session from this link <https://drive.google.com/open?id=0B_Gkb_0tNKkQUVc5NUhKRzlhZTg>

Before you start this session, it is important you have (i) the latest [R software] (https://cran.r-project.org/bin/windows/base/) and (ii) [R studio](https://www.rstudio.com/) installed in your computer.
...
require("knitr")
opts_knit$set(root.dir = "C:/Users/jymutua/Documents/LDN_Workshop/Sample_dataset/NDVI_data")
```
...
```{r}
setwd("C:/Users/jymutua/Documents/LDN_Workshop/Sample_dataset/NDVI_data")
```
...
Import all NDVI raster datasets in your working directory and create a raster stack
```{r}
NDVI_Otji_stack <- stack(list.files(getwd(), full.names = TRUE, pattern = ".tif$"))
```

Insert a Temporal id to the data frame. Characters corresponding to the year are located between the 20 and 23 position, while characters corresponding to the date are located between the 24 and 26 position
```{r}
...
```{r}
ggsave(file="Otjiwarongo_Monthly_NDVI.pdf", width = 297, height = 210, units = "mm")
```
...
Plot the same plot as before but with one plot per season, save as .pdf
```{r}
ggsave(file="Otjiwarongo_Hot_Wet_NDVI.pdf", width = 297, height = 210, units = "mm")
```
...
Plot the same plot as before but with one plot per season, save as .pdf
```{r}
ggsave(file="Otjiwarongo_Cool_Dry_NDVI.pdf", width = 297, height = 210, units = "mm")
```
...
Plot the same plot as before but with one plot per season, save as .pdf
```{r}
ggsave(file="Otjiwarongo_Hot_Dry_NDVI.pdf", width = 297, height = 210, units = "mm")
```
...
ggsave(file="Otjiwarongo_Seasonal_NDVI.pdf", width = 297, height = 210, units = "mm")
```
