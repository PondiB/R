...
Let's plot teh two datasets i.e. training and covariates
```{r}
plot(trainData)
plot(rstack[1],add=TRUE)
```
...
```{r}
predict(covs, model1, filename="otji_bd1", format='GTiff', type="response", 
...
```{r}
predict(covs, model2, filename="otji_bd2", format='GTiff', type="response", 
        index=1, na.rm=TRUE, progress="window", overwrite=TRUE)
...
```{r}
predict(covs, model3, filename="otji_bd3", format='GTiff', type="response", 
        index=1, na.rm=TRUE, progress="window", overwrite=TRUE)
