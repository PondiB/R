...
[site](http://earthexplorer.usgs.gov/). Otherwise,
...
Calculate NDVI and save the output for use in the next session. The resulting 
NDVI can be plotted as a map, values range from 0.2 (bare soils) to 0.6 (dense 
...
('Otji_trainingData.shp') which stores the id for each land cover type in a 
column in the attribute table called ‘LC_Code’ as shown below:
...
We can now partition the data into training and testing, this will enable us 
...
As you can see below, the training and testsing 'data.frames' contains values 
...
from the 'caret' package by specifying the model as a formula with the dependent
